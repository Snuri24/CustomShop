...
public class Render {
	...
	public static void drawItemStack(ItemStack itemStack, int x, int y) {
		...
		mc.getItemRenderer().renderAndDecorateItem(mc.player, itemStack, x, y);
	...
	public static void drawItemStack(ItemStack itemStack, double x, double y, double width, double height) {
		...
		RenderSystem.scaled(scaleX, scaleY, 1.0D);
		Render.drawItemStack(itemStack, (int) (x / scaleX), (int) (y / scaleY));
		RenderSystem.popMatrix();
	}
	
	public static void drawTooltip(MatrixStack matrixStack, ItemStack itemStack, int x, int y) {
		Minecraft mc = Minecraft.getInstance();
		
		FontRenderer font = itemStack.getItem().getFontRenderer(itemStack);
		GuiUtils.preItemToolTip(itemStack);
		List<ITextComponent> tooltips = itemStack.getTooltipLines(mc.player, mc.options.advancedItemTooltips ? ITooltipFlag.TooltipFlags.ADVANCED : ITooltipFlag.TooltipFlags.NORMAL);
		GuiUtils.drawHoveringText(matrixStack, tooltips, x, y, mc.screen.width, mc.screen.height, -1, (font == null ? mc.font : font));
